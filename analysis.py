...
def write_svg_bar_line_chart(
    bar_series: Dict[str, List[float]],
    growth_series: Dict[str, List[float]],
    years: List[int],
    path: str,
) -> None:
    """Render a 2x2 dashboard combining bar (absolute) and line (growth) series."""
    ...
    for idx, (title, values) in enumerate(items):
        ...
        scale = plot_height / max_value if max_value else 0.0

        growth_values = growth_series.get(title, [math.nan for _ in years])
        growth_valid = [v for v in growth_values if not math.isnan(v)]
        if growth_valid:
            line_min = min(growth_valid)
            line_max = max(growth_valid)
            if math.isclose(line_min, line_max):
                delta = abs(line_min) * 0.1 or 1.0
                line_min -= delta
                line_max += delta
        else:
        ...
        def line_y(value: float) -> float:
            if line_range == 0:
                return y0 + padding + plot_height / 2
            return y0 + padding + plot_height - (value - line_min) / line_range * plot_height
        ...
        svg_parts.append(
            f'<text x="{x0 + 16}" y="{y0 + 48}" text-anchor="start" fill="#666">柱状：指标值，折线：同比(%)</text>'
        )

        # Axes
        axis_x = y0 + chart_height - padding
        axis_y = x0 + padding
        right_axis_x = axis_y + plot_width
        svg_parts.append(
            f'<line x1="{axis_y}" y1="{axis_x}" x2="{axis_y + plot_width}" y2="{axis_x}" stroke="#444" stroke-width="1" />'
        )
        svg_parts.append(
            f'<line x1="{axis_y}" y1="{y0 + padding}" x2="{axis_y}" y2="{axis_x}" stroke="#444" stroke-width="1" />'
        )
        svg_parts.append(
            f'<line x1="{right_axis_x}" y1="{axis_x}" x2="{right_axis_x}" y2="{y0 + padding}" stroke="#d0021b" stroke-width="1" stroke-dasharray="4 2" />'
        )

        if line_min < 0 < line_max:
            zero_y = line_y(0.0)
            svg_parts.append(
                f'<line x1="{axis_y}" y1="{zero_y}" x2="{right_axis_x}" y2="{zero_y}" stroke="#d0021b" stroke-width="1" stroke-dasharray="2 2" opacity="0.6" />'
            )
        ...
        # Line series for growth
        prev_point: Optional[Tuple[float, float]] = None
        for i, year in enumerate(years):
            growth = growth_values[i]
            if math.isnan(growth):
                prev_point = None
                continue
            cx = axis_y + i * step + step / 2
            cy = line_y(growth)
            if prev_point is not None:
                svg_parts.append(
                    f'<line x1="{prev_point[0]}" y1="{prev_point[1]}" x2="{cx}" y2="{cy}" stroke="#d0021b" stroke-width="2" opacity="0.85" />'
                )
            svg_parts.append(f'<circle cx="{cx}" cy="{cy}" r="4" fill="#fff" stroke="#d0021b" stroke-width="2" />')
            prev_point = (cx, cy)

        # Axis annotations
        svg_parts.append(
            f'<text x="{axis_y}" y="{y0 + padding - 8}" text-anchor="start">最大值: {max_value:.0f}</text>'
        )
        svg_parts.append(
            f'<text x="{right_axis_x + 6}" y="{y0 + padding + 12}" text-anchor="start" fill="#d0021b">{line_max:.1f}%</text>'
        )
        mid_label = 0.0 if line_min < 0 < line_max else (line_min + line_max) / 2
        mid_y = line_y(mid_label)
        svg_parts.append(
            f'<text x="{right_axis_x + 6}" y="{mid_y + 5}" text-anchor="start" fill="#d0021b">{mid_label:.1f}%</text>'
        )
        svg_parts.append(
            f'<text x="{right_axis_x + 6}" y="{axis_x}" text-anchor="start" fill="#d0021b">{line_min:.1f}%</text>'
        )
...
def main() -> None:
    ...
    macro_growth = {}
    macro_growth_mapping = {
        "常住人口(万人)": "population",
        "GDP(亿元)": "gdp",
        "能源消费量(万tce)": "energy",
        "碳排放量(万tCO2)": "carbon",
    }
    for label, key in macro_growth_mapping.items():
        growth_dict = yoy[key]
        converted: List[float] = []
        for year in YEARS:
            value = growth_dict.get(year)
            if value is None or math.isnan(value):
                converted.append(math.nan)
            else:
                converted.append(value * 100)
        macro_growth[label] = converted

    write_svg_bar_line_chart(
        macro_series,
        macro_growth,
        YEARS,
        os.path.join("figures", "macro_indicators_bar_line.svg"),
    )
